  ...
  private List<Chromosome> createTwoNewChromosomes(Chromosome parent1, Chromosome parent2) {
    ...
    for (int i = 0; i < similarityVector.length; i++) {
      if (similarityVector[i] == Boolean.TRUE) {
        ...
        firstChild.get(i / Double.SIZE).set(i % Double.SIZE, RandomUtils.RANDOM.nextBoolean());
        secondChild.get(i / Double.SIZE).set(i % Double.SIZE, RandomUtils.RANDOM.nextBoolean());
      }
    }
    return List.of(
